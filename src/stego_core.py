...
MAGIC = b"STEG"                 # Identifies images created by this tool
HEADER_LENGTH_BYTES = 4         # Stores secret file size
HEADER_EXT_BYTES = 8            # Stores file extension
HEADER_TOTAL_BYTES = (
    len(MAGIC) + HEADER_LENGTH_BYTES + HEADER_EXT_BYTES
)
...
def load_cover_image(path):
    ...
    RGB guarantees exactly 3 channels:
    Red, Green, Blue.
    """
    path = Path(path)
...
def bytes_to_bits(data: bytes) -> np.ndarray:
    """Convert bytes into an array of 0/1 bits."""
    if not data:
...
def bits_to_bytes(bits: np.ndarray) -> bytes:
    """Convert an array of bits back into bytes."""
    if len(bits) == 0:
        return b""

    # A valid byte sequence must contain complete groups of 8 bits.
    if len(bits) % 8 != 0:
...
def build_header(secret_bytes: bytes, extension: str) -> bytes:
    ...
    # Store secret length using 4 bytes.
    length_field = len(secret_bytes).to_bytes(
        HEADER_LENGTH_BYTES,
        byteorder="big"
    )
    ...
    if len(ext_bytes) > HEADER_EXT_BYTES:
        raise ValueError(
            f"Extension '{extension}' is too long."
        )

    # Pad extension to exactly 8 bytes.
...
def parse_header(header_bytes: bytes):
    ...
    if magic != MAGIC:
        raise ValueError(
            "This does not appear to be a valid stego image "
            "created by this tool."
        )

    # Read secret file length.
    ...
    if extension and extension not in SUPPORTED_EXTENSIONS:
        raise ValueError(
            f"Unsupported or invalid extension: {extension}"
        )
...
def calculate_capacity_bytes(img: Image.Image) -> int:
    """
    Calculate the total number of bytes that can be stored.
...
def calculate_available_secret_capacity(img: Image.Image) -> int:
    """
    Calculate how many bytes are available specifically
    for the secret file after reserving space for the header.
    """

    return max(
        0,
        calculate_capacity_bytes(img) - HEADER_TOTAL_BYTES
    )
...
def embed(
    ...
    """
    Hide a secret file inside a cover image using LSB.

    Returns a dictionary containing information useful
    for the GUI and report.
    """
    ...
    # Clear the LSB and insert secret bit.
    flat[:len(bits)] = (
        (flat[:len(bits)] & 0xFE) |
        bits
    )
...
def extract(
    ...
    """
    Extract a hidden secret file from a stego image.

    Returns a dictionary containing extraction information.
    """
    ...
    if not stego_path.exists():
        raise FileNotFoundError(
            f"Stego image not found: {stego_path}"
        )
    ...
    if len(flat) < header_bits_needed:
        raise ValueError(
            "Image is too small to contain a valid stego header."
        )
    ...
    if end > len(flat):
        raise ValueError(
            ...
            "The declared payload exceeds image capacity."
        )
    ...
    output_path = (
        output_dir / f"extracted_secret{extension}"
    )
...
def calculate_mse(cover_path: str, stego_path: str) -> float:
    ...
    Lower MSE = smaller pixel differences.
    """

    cover = np.array(
        load_cover_image(cover_path),
        dtype=np.float64
    )

    stego = np.array(
        load_cover_image(stego_path),
        dtype=np.float64
    )
...
def calculate_psnr(cover_path: str, stego_path: str) -> float:
    ...
    Higher PSNR generally means better visual similarity.
    """
...
def calculate_histogram(image_path: str):
    ...
    red_histogram = np.histogram(
        pixels[:, :, 0],
        bins=256,
        range=(0, 256)
    )[0]

    green_histogram = np.histogram(
        pixels[:, :, 1],
        bins=256,
        range=(0, 256)
    )[0]

    blue_histogram = np.histogram(
        pixels[:, :, 2],
        bins=256,
        range=(0, 256)
    )[0]
...
def analyze_file_size(
    ...
    """
    Compare cover and stego image file sizes.
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            ...
            f"{result['remaining_capacity']:,} bytes"
        )
        ...
        print(
            f"Cover size        : "
            f"{size_info['cover_size']:,} bytes"
        )
        print(
            f"Stego size        : "
            f"{size_info['stego_size']:,} bytes"
        )
        print(
            f"Difference        : "
            f"{size_info['difference']:+,} bytes"
        )
        print(
            f"Percentage change : "
            f"{size_info['percentage_change']:+.2f}%"
        )
